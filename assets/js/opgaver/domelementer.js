...
redFigure.addEventListener("click", () => {
  redFigure.classList.toggle("blueBackground");
});
/* opgave 1.2
...
yellowFigure.addEventListener("click", () => {
  yellowFigure.classList.toggle("blueBackground");
});
/* opgave 2.1
...
*/
const yeet = document.querySelectorAll(".purpleFigures");
for (let index = 0; index < yeet.length; index++) {
  console.log(yeet[index]);
  yeet[index].addEventListener("click", (data) => {
    changeDiv(data.target);
  });
}
function changeDiv(target) {
  console.log(target);
  target.classList.toggle("orgin");
}
// din kode her
